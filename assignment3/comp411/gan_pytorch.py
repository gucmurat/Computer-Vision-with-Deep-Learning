...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss = bce_loss(logits_fake, torch.zeros_like(logits_fake).type(dtype).squeeze()) + bce_loss(logits_real, torch.ones_like(logits_real).type(dtype).squeeze())
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = 0.5 * torch.mean((scores_real - torch.ones_like(scores_real).type(dtype).squeeze()) ** 2) + 0.5 * torch.mean((scores_fake - torch.zeros_like(scores_fake).type(dtype).squeeze()) ** 2)  
...
def ls_generator_loss(scores_fake):
    ...
    loss = 0.5 * torch.mean((scores_fake.squeeze() - torch.ones_like(scores_fake).type(dtype).squeeze()) ** 2)
